...
const createProductReview = asyncHandler(async (req, res) => {
  ...
  if (product) {
    ...
    if (alreadyReviewed) {
      res.status(400);
      throw new Error('Este producto ya tiene una reseña tuya');
    }
...
const createNewProduct = asyncHandler(async (req, res) => {
  ...
  if (newProduct) {
    ...
    res.status(404);
    throw new Error('El producto no ha podido crearse.');
  }
});
...
const removeProductReview = asyncHandler(async (req, res) => {
  ...
  if (product) {
    ...
    res.status(201).json({ message: 'La reseña ha sido eliminada.' });
  } else {
    res.status(404);
    throw new Error('No se ha encontrado el producto');
  }
